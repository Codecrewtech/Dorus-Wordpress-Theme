...
function spyropress_wc_product_thumbnail() {
	...
	if ( has_post_thumbnail() ) {
		...
		<div class="product-img">
			<?php get_image( array( 'class' => 'img-responsive' ) ); ?>
		</div>  
		<?php 
...
function woocommerceframework_pagination_top() {
	$features = get_setting_array( 'shop_loop_top_bar', array() );
	$pagination_pos = get_setting( 'shop_pagination_pos', 'bottom' );
...
add_action( 'woocommerce_before_shop_loop', 'woocommerceframework_pagination_top', 1 );
...
/** Remove and Re-position rating and price */
...
function spyropress_wc_related_products_args( $args ) {
	...
	$args['posts_per_page'] = ( isset( $_GET['shop_layout']  ) )? str_replace( '4', '3', $args['posts_per_page'] ) : $args['posts_per_page'];
	$args['columns'] = get_setting( 'shop_related_columns', $args['columns'] );
	$args['columns'] = ( isset( $_GET['shop_layout']  ) )? str_replace( '4', '3', $args['columns'] ) : $args['columns'];
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if ( ! empty( $args['custom_attributes'] ) && is_array( $args['custom_attributes'] ) )
			...
			if ( ! empty( $args['validate'] ) )
				...
				switch ( $args['type'] ) {
					...
					if ( sizeof( $countries ) == 1 ) {
						...
						. '<option value="">'.__( 'Select a country&hellip;', 'tomato' ) .'</option>';
						...
						$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_html__( 'Update country', 'tomato' ) . '" /></noscript>';
					...
					if ( is_array( $states ) && empty( $states ) ) {
						...
						$field  = '<div class="row ' . esc_attr( implode( ' ', $args['class'] ) ) .'" id="' . esc_attr( $key ) . '_field" style="display: none"><div class="col-md-12"><div class="form-group">';
						...
						$field  = '<div class="row ' . esc_attr( implode( ' ', $args['class'] ) ) .'" id="' . esc_attr( $key ) . '_field"><div class="col-md-12"><div class="form-group">';
						...
						<option value="">'.__( 'Select a state&hellip;', 'tomato' ) .'</option>';
					...
					if ( ! empty( $args['options'] ) )
						...
						$field .= '<input type="email" class="form-control ' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '" '.$args['maxlength'].' value="' . esc_attr( $value ) . '" ' . implode( ' ', $custom_attributes ) . ' /></div></div></div>' . $after;
						break;
						case 'tel' :
						$field = '<div class="row ' . esc_attr( implode( ' ', $args['class'] ) ) .'" id="' . esc_attr( $key ) . '_field"><div class="col-md-12"><div class="form-group">';
						if ( $args['label'] )
						...
						$field .= '<input type="tel" class="form-control ' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" name="' . esc_attr( $key ) . '" id="' . esc_attr( $key ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '" '.$args['maxlength'].' value="' . esc_attr( $value ) . '" ' . implode( ' ', $custom_attributes ) . ' /></div></div></div>' . $after;
						break;
						default :
			...
			add_filter( 'wc_add_to_cart_message_html', 'spyropress_wc_add_to_cart_message' );
...
add_filter( 'woocommerce_product_get_rating_html', function( $html, $rating ) {
	...
	$rating_html = '';	
	$rating_html .= '<div class="rc-ratings">';

	$rating_html .= str_repeat( '<span class="fa fa-star active"></span>', $rating );
	$rating_html .= str_repeat( '<span class="fa fa-star active"></span>', 5 - $rating );
...
if (!function_exists('woocommerce_template_single_title')) {
	...
	{ ?>
		<div class="shop-single-title">
			<h3 itemprop="name" class="text-left"><?php the_title(); ?></h3>
		</div>
	<?php }
}
