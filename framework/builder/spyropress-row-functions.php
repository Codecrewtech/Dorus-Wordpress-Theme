...
function builder_new_row( $args, $builder_data ) {
    ...
    $json['message'] = ( $result ) ? esc_html__( 'Row Saved', 'tomato' ) : esc_html__( 'Operation fails', 'tomato' );
    $json['html'] = ( $result ) ? $html : esc_html__( 'Oops! something goes wrong while creating the new row.', 'tomato' );
    $json['row_id'] = $row_ID;
...
function builder_delete_row( $args, $builder_data ) {
    ...
    if ( isset( $builder_data[$row_id] ) ) {
        ...
        // Generate json data
        $json['success'] = ( $result ) ? true : false;
        $json['message'] = ( $result ) ? esc_html__( 'Row Deleted', 'tomato' ) : esc_html__( 'Operation fails.', 'tomato' );
        $json['html'] = ( $result ) ? esc_html__( 'Row deleted successfully.', 'tomato' ) : esc_html__( 'Oops! something goes wrong while deleting the row.', 'tomato' );
        $json['row_id'] = $row_id;
    }
    // If row doesn't exists
        ...
        // Generate json data
        $json['success'] = false;
        $json['message'] = esc_html__( 'Row not exists', 'tomato' );
        $json['html'] = sprintf( esc_html__( 'Row: %s doesn\'t exists.', 'tomato' ), $row_id );
        $json['row_id'] = $row_id;
    }
...
function builder_reorder_rows( $args, $builder_data ) {
    ...
    // Generate json data
    $json['success'] = ( $result ) ? true : false;
    $json['message'] = ( $result ) ? esc_html__( 'Row Order Updated', 'tomato' ) : esc_html__( 'Operation fails', 'tomato' );
    $json['html'] = ( $result ) ? esc_html__( 'Row Order Updated', 'tomato' ) : esc_html__( 'Oops! something goes wrong while updating rows order.', 'tomato' );
...
function builder_edit_row( $args, $builder_data ) {
    ...
    $json['html'] = ( $result ) ? $html : esc_html__( 'Oops! something goes wrong while generating module form.', 'tomato' );
    $json['row_id'] = $row_id;
...
function builder_save_row( $args, $builder_data ) {
    ...
    // Generate json data
    $json['success'] = ( $result ) ? true : false;
    $json['message'] = ( $result ) ? esc_html__( 'Row Option Saved', 'tomato' ) : esc_html__( 'Operation fails', 'tomato' );
    $json['html'] = ( $result ) ? esc_html__( 'Saved', 'tomato' ) : esc_html__( 'Oops! something goes wrong while saving the row options, development team has been notified!', 'tomato' );
    $json['row_id'] = $row_id;
