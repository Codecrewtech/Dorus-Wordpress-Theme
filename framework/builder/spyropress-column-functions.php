...
function builder_new_column( $args, $builder_data ) {
    ...
    $json['message'] = ( $result ) ? esc_html__( 'Column Saved', 'tomato' ) : esc_html__( 'Operation fails', 'tomato' );
    $json['html'] = ( $result ) ? $html : esc_html__( 'Oops! something goes wrong while creating the new column.', 'tomato' );
    $json['row_id'] = $row_id;
    $json['col_id'] = $col_ID;
...
function builder_delete_column( $args, $builder_data ) {
    ...
    if ( isset( $builder_data[$row_id]['columns'][$col_id] ) ) {
        ...
        // Generate json data
        $json['success'] = ( $result ) ? true : false;
        $json['message'] = ( $result ) ? esc_html__( 'Column Deleted', 'tomato' ) : esc_html__( 'Operation fails', 'tomato' );
        $json['html'] = ( $result ) ? $html : esc_html__( 'Oops! something goes wrong while deleting the column.', 'tomato' );
        $json['row_id'] = $row_id;
        $json['col_id'] = $col_id;
    ...
    // If column doesn't exists
        ...
        // Generate json data
        $json['success'] = false;
        $json['message'] = esc_html__( 'Column not exists', 'tomato' );
