...
function builder_edit_module($args, $builder_data)
{
    ...
    $json = [];
    if (false !== $instance) {
        $callback_types = ['form', 'render_media'];
        foreach ($callback_types as $callback)
            if (is_callable([$widget, $callback])) {
                $form = $widget->$callback($instance);
            }
        $widget_slug = strtolower($module_type);
        $widget_slug = str_replace('wp_widget_', '', $widget_slug);
        $widget_slug = str_replace('_', '-', $widget_slug);
        if (is_callable([$widget,'render_control_template_scripts']) && strpos($widget_slug,'media') !== false) {
            $widget->render_control_template_scripts();
            $widget_slug = str_replace('media_','',$widget_slug);
            $json['template_id'] = "wp-media-widget-{$widget_slug}-control-fields";
        } elseif (is_callable([$module_type, 'render_control_template_scripts'])) {
    ...
    $json['message'] = ($result) ? esc_html__('Returning form', 'tomato') : esc_html__('Operation fails', 'tomato');
    $json['html'] = ($result) ? $form_html : esc_html__('Oops! something goes wrong while generating module form.', 'tomato');
    $json['row_id'] = $row_id;
    $json['col_id'] = $col_id;
...
function builder_save_module($args, $builder_data)
{
    ...
    // Generate json data
    $json['success'] = ($result) ? true : false;
    $json['message'] = ($result) ? esc_html__('Module Saved', 'tomato') : esc_html__('Operation fails', 'tomato');
    $json['html'] = ($result) ? $module_html : esc_html__('Oops! something goes wrong while saving the new module, development team has been notified!', 'tomato');
    $json['row_id'] = $row_id;
    $json['col_id'] = $col_id;
...
function builder_delete_module($args, $builder_data)
{
    ...
    if (isset($builder_data[$row_id]['columns'][$col_id]['modules'][$module_id])) {
        ...
        // Generate json data
        $json['success'] = ($result) ? true : false;
        $json['message'] = ($result) ? esc_html__('Module Deleted', 'tomato') : esc_html__('Operation fails', 'tomato');
        $json['html'] = ($result) ? esc_html__('Module deleted successfully.', 'tomato') : esc_html__('Oops! something goes wrong while deleting the module, development team has been notified!', 'tomato');
        $json['row_id'] = $row_id;
        $json['col_id'] = $col_id;
        $json['module_id'] = $module_id;
...
function builder_reorder_modules($args, $builder_data)
{
    ...
    // Generate json data
    $json['success'] = true;
    $json['message'] = ($result) ? esc_html__('Module Order Updated', 'tomato') : esc_html__('Operation fails', 'tomato');
    $json['html'] = ($result) ? esc_html__('Module Order Updated.', 'tomato') : esc_html__('Oops! something goes wrong while updating modules order.', 'tomato');
...
function builder_clone_module($args, $builder_data)
{
    ...
    if (isset($builder_data[$row_id]['columns'][$col_id]['modules'][$module_id])) {
        ...
        // Generate json data
        $json['success'] = ($result) ? true : false;
        $json['message'] = ($result) ? esc_html__('Module Cloned', 'tomato') : esc_html__('Operation fails', 'tomato');
        $json['html'] = ($result) ? $module_html : esc_html__('Oops! something goes wrong while deleting the module, development team has been notified!', 'tomato');
        $json['row_id'] = $row_id;
        $json['col_id'] = $col_id;
        ...
        // Generate json data
        $json['success'] = false;
        $json['message'] = esc_html__('Module not exists', 'tomato');
        $json['html'] = 'Module: ' . $module_id . ' doesn\'t exists.';
        $json['row_id'] = $row_id;
        $json['col_id'] = $col_id;
        $json['module_id'] = $module_id;
