...
class lessc {
	...
	protected function tryImport($importPath, $parentBlock, $out) {
		if (!function_exists('file_cpt_get_contents')) return;
		...
		$root = $parser->parse(file_cpt_get_contents($realPath));
	...
	protected function lib_data_uri($value) {
		if(!function_exists('base64_cpt_encode')) return;
		...
		if($fullpath && ($fsize = filesize($fullpath)) !== false) {
			...
			if($fsize/1024 < 32) {
				...
				if(!is_null($mime)) // fallback if the mime type is still unknown
					$url = sprintf('data:%s;base64,%s', $mime, base64_cpt_encode(file_cpt_get_contents($fullpath)));
			}
		}
	...
	public function compileFile($fname, $outFname = null) {
		...
		$out = $this->compile(file_cpt_get_contents($fname), $fname);

		$this->importDir = $oldImport;

		if ($outFname !== null) {
			return file_cpt_put_contents($outFname, $out);
		}
