...
class Spyropress {
    ...
    function frontend_includes() {
        ...
        get_template_part ( 'framework/spyropress', 'image' ); // Load all shortcodes into system
        get_template_part ( 'includes/shortcodes/spyropress-shortcode', 'init' );
    }
        ...
        if ( $footer_layout ) {
            ...
            for ( $i = 0; $i < $footer_count; $i++ )
                $sidebars['footer-' . ( $i + 1 )] = array(
                    'name' => esc_html__( 'Footer ', 'tomato' ) . ( $i + 1 ),
                    'description' => esc_html__( 'A widget area loaded in the footer of the site.', 'tomato' )
                );
        }
    ...
    function browser_body_class() {
        ...
        elseif ( $is_IE ) {
			...
			if (function_exists('sprypress_cpt_user_agent')) {
				// Version Info
				$browser = sprypress_cpt_user_agent();
