 ...
 * Improved Excerpt
 */
function spyropress_get_excerpt( $args = '', $post_id = '' ) {
    ...
    if ( $link_to_post ) {
        ...
        $text .= ' <a class="btn btn-default" href="' . $permalink . '">' . $link_text . '</a>';
    }
...
function spyropress_query_generator( $atts, $content = null ) {
    ...
    $output = '';
    $counter = $column_counter = $custom_post_counter = 0;
    $columns = $atts['columns'];
    $callback = $atts['callback'];
    $close = false;
    ...
    if ( $posts->have_posts() ) {
        while ( $posts->have_posts() ) {
            $posts->the_post();
            $custom_post_counter = $posts->post_count;;
            $counter++;
            ...
            if ( $columns ) {
                ...
                if ( $column_counter == $columns ) {
                    ...
                    if ( $atts['row'] ) {
                        $output .= '</' . $atts['row_container'] . '>';
                        if( isset( $atts['divider'] ) && $custom_post_counter != $counter ){
                            $output .= '<hr />';
                        }
                        $close = false;
                    }
...
function spyropress_page_wrapper() {
    //get page option meta data value.
    $spyropress_options = get_post_meta( get_the_ID(), '_page_options', true );
    
    $wrapper = '<!-- wrapper -->' . "\n";
    if( !empty( $spyropress_options ) && isset( $spyropress_options['spyropress_bucket'] ) && !empty( $spyropress_options['spyropress_bucket'] ) ){
       $wrapper .= spyropress_get_the_bucket( $spyropress_options['spyropress_bucket'] );
    }  
    if ( 'boxed' == get_setting( 'theme_layout' ) )
        $wrapper .= '<div class="body boxed"><div class="main-wrapper">';
    else
        $wrapper .= '<div class="body"><div class="main-wrapper">';
...
function display_browser_happy() {
    echo '<!--[if lt IE 8]>';
    echo '<p class="chromeframe">' . esc_html__( 'You are using an <strong>outdated</strong> browser. Please upgrade your browser or activate Google Chrome Frame to improve your experience.', 'tomato' ) . '</p>';
    echo '<![endif]-->';
    echo "\n";
}
...
function output_credit() {
    echo '<!-- ' . esc_html__( 'Powered by WordPress and the SpyroPress Framework', 'tomato' ) . ' -->' . "\n";
}
