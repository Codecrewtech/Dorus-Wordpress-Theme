...
function get_themeforest_username() { return 'tomato'; }
function get_documentation_link( $text = '' ) { $text = ( $text ) ? $text : esc_html__( 'Documentation', 'tomato' ); echo '<a target="_blank" href="http://spyropress.com/codex/">' . $text . '</a>'; }
function get_support_forum_link( $text = '' ) { $text = ( $text ) ? $text : esc_html__( 'Forum', 'tomato' ); echo '<a target="_blank" href="http://themesquared.zendesk.com/hc/en-us/">' . $text . '</a>'; }
function get_suggest_link() { echo '<a target="_blank" href="http://spyropress.com/contact/">' . esc_html__( 'Suggest a Feature', 'tomato' ) . '</a>'; }
function get_showcase_link() { echo '<a target="_blank" href="http://spyropress.com/contact/">' . esc_html__( 'Showcase your Website', 'tomato' ) . '</a>'; }
...
function get_spyropress_badge( $class = '' ) {
    printf( '<div class="wp-badge%2$s">' . esc_html__( 'Version %1$s', 'tomato' ) . '</div>', spyropress_get_version(), ( $class ) ? ' ' . $class : '' );
}
...
function spyropress_get_theme_update_notice( $theme_name, $theme_version, $latest_version ) {
    ...
    echo '<li>' . sprintf( esc_html__( '<strong>There is a new version of the %s theme available.</strong> You have version %s installed. Update to version %s',
        'tomato' ), $theme_name, $theme_version, $latest_version ) . '</li>';
    echo '</ul>';
}
...
function build_section_reset() {
    echo '<a href="#" class="section-reset">' . esc_html__( 'Reset', 'tomato' ) . '</a>';
}
