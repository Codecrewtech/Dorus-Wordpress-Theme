    ...
    echo '<div ' . build_section_class( 'section-upload', $item ) . '>';
        ...
        echo '<div class="controls">';
            echo '<div class="uploader clearfix">';
                ...
                printf( '<input class="upload_button button-secondary" type="button" value="%2$s" id="upload_%1$s" />', esc_attr( $id ), esc_html__( 'Upload', 'tomato' ) );
                if ( is_array( @getimagesize( $value ) ) ) {
                    print '<div class="screenshot" id="' . esc_attr( $id ) . '_image">';
                    if ( '' != $value ) {
                        $remove = '<a href="javascript:(void);" class="remove-media">' . esc_html__( 'Remove', 'tomato' ) . '</a>';
...
function spyropress_ui_gallery( $item, $id, $value, $is_widget = false, $is_builder = false ) {
    ...
    echo '<div ' . build_section_class( 'section-gallery section-full', $item ) . '>';
        printf( '<input class="gallery_reset_button button-red button-large pull-right" type="button" value="%s" /> ', esc_html__( 'Clear Gallery', 'tomato' ) );
        printf( '<input class="gallery_upload_button button button-primary button-large right" type="button" value="%2$s" id="upload_%1$s" />', esc_attr( $id ), esc_html__( 'Add/Edit Gallery', 'tomato' ) );
        build_heading( $item, $is_widget );
        build_description( $item );
        echo '<div class="controls">';
