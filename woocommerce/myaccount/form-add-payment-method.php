    ...
    <form id="add_payment_method" method="post">
    	<div id="payment">
    			...
    			<?php
					...
					foreach ( $available_gateways as $gateway ) {
						...
						<li class="woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $gateway->id ); ?> payment_method_<?php echo esc_attr( $gateway->id ); ?>">
							...
							<label for="payment_method_<?php echo esc_attr( $gateway->id ); ?>"><?php echo esc_html( $gateway->get_title() ); ?> <?php echo tomato_html($gateway->get_icon()); ?></label>
							<?php
    		...
    		<div class="form-row">
    			...
    			<input type="submit" class="button alt button btn-lg no-width u-pull-right alt" id="place_order" value="<?php _e( 'Add Payment Method', 'tomato' ); ?>" />
...
<?php else : ?>
	<p class="woocommerce-notice woocommerce-notice--info woocommerce-info alert alert-info"><?php esc_html_e( 'Sorry, it seems that there are no payment methods which support adding a new payment method. Please contact us if you require assistance or wish to make alternate arrangements.', 'tomato' ); ?></p>
<?php endif; ?>
